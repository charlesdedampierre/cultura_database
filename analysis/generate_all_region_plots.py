...
Saves plots to dynasty_plots/ directory.

Comprehensive coverage of every major region in the world.
"""
import sqlite3
import math
import os
...
def main():
    ...
    log("=== Generating all world region plots ===")
    ...
    for key in sorted(ALL_REGIONS.keys()):
        ...
        log(f"\n--- {name} ---")
        data = load_region_data(conn, polities)
    ...
    # Print final summary
    log(f"\n{'='*80}")
    log(f"  SUMMARY: All World Regions")
    log(f"{'='*80}")

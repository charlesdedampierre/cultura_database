/// Create individuals_regions table.
...
///
/// This is a standalone association table, separate from individuals_countries.
use anyhow::Result;
...
fn main() -> Result<()> {
    ...
    log("=== Step 43: Create individuals_regions ===");
    ...
    log("[43] Building region lookup from regions table...");
    ...
    log(&format!(
        "[43] Region lookup: {} country codes with region mappings",
        region_lookup.len()
    ));

    // Build impact_date lookup: wikidata_id -> year
    log("[43] Building impact_date lookup...");
    ...
    // ========================================================
    log("[43] Creating fresh individuals_regions table...");
    ...
    log("[43] Created fresh individuals_regions table");
    ...
    log(&format!(
        "[43] Total rows in individuals_countries to process: {}",
        total
    ));
    ...
    loop {
        // Read a batch from individuals_countries
        ...
        conn.execute_batch("BEGIN TRANSACTION;")?;
        {
            ...
            for (wikidata_id, name_en, iso_country_name, iso_a3, origins) in &batch {
                ...
                if let Some(&year) = impact_year {
                    ...
                    // No impact date: still insert with NULL region/macro_region/impact_year
                    // so we don't lose individuals from the table
                    no_impact_date += 1;
                }
            }
        }
        ...
        if offset % 500_000 < BATCH_SIZE as i64 {
            log(&format!(
                "[43] Progress: {}/{} processed, {} inserted, {} no region match, {} no impact date",
    ...
    log("[43] === Final Statistics ===");
    log(&format!("[43] Total individuals_countries: {}", total));
    log(&format!("[43] Total in individuals_regions: {}", final_count));
    log(&format!("[43] With region assigned: {}", with_region));
    log(&format!("[43] Matched to region: {}", matched_count));
    log(&format!(
        "[43] No region match (country not in regions table): {}",
        unmatched_no_region
    ));
    log(&format!("[43] No impact date available: {}", no_impact_date));
    ...
    let rows: Vec<(String, i64)> = stmt
        ...
        .filter_map(|r| r.ok())
        .collect();
    log("[43] Top 20 regions:");
    for (reg, cnt) in &rows {
        log(&format!("[43]   {} -> {}", reg, cnt));
    }
    ...
    let rows: Vec<(String, i64)> = stmt
        ...
        .filter_map(|r| r.ok())
        .collect();
    log("[43] Macro regions:");
    for (mr, cnt) in &rows {
        log(&format!("[43]   {} -> {}", mr, cnt));
    }

    log("=== Step 43 complete ===");
    Ok(())
}

...
JSON_PATH = (
    ROOT / "data" / "all_humans" / "wikidata_extraction_scripts_v2" / "place_dates.json"
)
...
def report(con: duckdb.DuckDBPyConnection) -> None:
    ...
    n_inc = con.execute(
        "SELECT COUNT(*) FROM places WHERE inception_date IS NOT NULL"
    ).fetchone()[0]
    n_dis = con.execute(
        "SELECT COUNT(*) FROM places WHERE dissolution_date IS NOT NULL"
    ).fetchone()[0]
    n_any = con.execute(
...
def main():
    ...
    if args.bench:
        ...
        with duckdb.connect(":memory:") as con:
            ...
            print(
                f"  bench: wrote {n:,} of {len(sample):,} sample rows in {elapsed:.2f}s"
            )

/// Rebuild individuals_countries with impact_year column.
///
/// Priority order for country assignment:
/// 1. nationality-location (via nationalities_en field, split by "; ")
/// 2. birth-location (via birthcity_en field)
/// 3. death-location (via deathcity_en field)
///
/// impact_year is looked up from individuals_impact_date and stored alongside
...
fn main() -> Result<()> {
    ...
    log("=== Step 55: Rebuild individuals_countries with impact_year ===");
    ...
    log("[55] Building nationality lookup...");
    ...
    log(&format!("[55] Nationality lookup: {} entries", nat_lookup.len()));

    // Build city name -> (iso_country_name, iso_a3_code) lookup
    log("[55] Building city lookup...");
    ...
    log(&format!("[55] City lookup: {} entries", city_lookup.len()));

    // Build impact_date lookup: wikidata_id -> year
    log("[55] Building impact_date lookup...");
    ...
    log(&format!("[55] Impact date lookup: {} entries", impact_lookup.len()));

    // ========================================================
    // PHASE 2: Drop and recreate individuals_countries table
    // ========================================================
    log("[55] Creating fresh individuals_countries table...");
    ...
    log("[55] Created fresh individuals_countries table with impact_year column");

    // ========================================================
    // PHASE 3: Populate from individuals table
    // ========================================================
    let total: i64 = conn.query_row("SELECT COUNT(*) FROM individuals", [], |r| r.get(0))?;
    log(&format!("[55] Total individuals to process: {}", total));
    ...
    loop {
        ...
        conn.execute_batch("BEGIN TRANSACTION;")?;
        {
            let mut insert = conn.prepare_cached(
                "INSERT OR IGNORE INTO individuals_countries (wikidata_id, name_en, iso_country_name, iso_a3_code, origins, impact_year)
                 VALUES (?1, ?2, ?3, ?4, ?5, ?6)",
            )?;

            for (wikidata_id, name_en, nationalities_en, birthcity_en, deathcity_en) in &batch {
                ...
                if let Some((country, iso, origin)) = found_country {
                    let impact_year = impact_lookup.get(wikidata_id.as_str()).copied();
        ...
        if offset % 500_000 < BATCH_SIZE as i64 {
            log(&format!(
                "[55] Progress: {}/{} processed, {} inserted (nat:{}, birth:{}, death:{}), {} unmatched",
                offset, total, total_inserted, matched_nationality, matched_birth, matched_death, unmatched
            ));
        }
    }
    pb.finish();
    ...
    conn.execute_batch("CREATE INDEX IF NOT EXISTS idx_indcountries_impact_year ON individuals_countries(impact_year);")?;
    ...
    log("[55] === Final Statistics ===");
    log(&format!("[55] Total individuals: {}", total));
    log(&format!("[55] Total in individuals_countries: {}", final_count));
    log(&format!("[55]   via nationality: {}", matched_nationality));
    log(&format!("[55]   via birthplace: {}", matched_birth));
    log(&format!("[55]   via deathplace: {}", matched_death));
    log(&format!("[55]   with impact_year: {}", with_impact_year));
    log(&format!("[55] Unmatched (no country): {}", unmatched));
    ...
    let rows: Vec<(String, String, i64)> = top
        ...
        .filter_map(|r| r.ok())
        .collect();
    log("[55] Top 15 countries:");
    for (name, iso, cnt) in &rows {
        log(&format!("[55]   {} ({}) -> {}", name, iso, cnt));
    }

    log("=== Step 55 complete ===");
    Ok(())
}

...
const DB_PATH: &str = "data/humans_clean_new.sqlite3";
...
fn main() -> Result<()> {
    ...
    log("=== Step 33: Create individuals_countries ===");
    ...
    log("[33] Building nationality lookup...");
    ...
    log(&format!("[33] Nationality lookup: {} entries", nat_lookup.len()));

    // Build city name -> (iso_country_name, iso_a3_code) lookup
    log("[33] Building city lookup...");
    ...
    log(&format!("[33] City lookup: {} entries", city_lookup.len()));

    // Build region lookup: iso_a3 -> Vec<RegionEntry>
    log("[33] Building region lookup...");
    ...
    log(&format!("[33] Region lookup: {} ISO codes", region_lookup.len()));

    // Build impact_date lookup: wikidata_id -> year
    log("[33] Building impact_date lookup...");
    ...
    log(&format!("[33] Impact date lookup: {} entries", impact_lookup.len()));

    // ========================================================
    // PHASE 2: Drop and recreate individuals_countries table
    // ========================================================
    log("[33] Creating fresh individuals_countries table...");
    ...
    loop {
        ...
        if offset % 500_000 < BATCH_SIZE as i64 {
            log(&format!(
                "[33] Progress: {}/{} processed, {} inserted (nat:{}, death:{}, birth:{}), {} with region, {} unmatched",
    ...
    log("[33] === Final Statistics ===");
    log(&format!("[33] Total individuals: {}", total));
    log(&format!("[33] Total in individuals_countries: {}", final_count));
    log(&format!("[33]   via nationality: {}", matched_nationality));
    log(&format!("[33]   via deathplace: {}", matched_death));
    log(&format!("[33]   via birthplace: {}", matched_birth));
    log(&format!("[33] With region: {}", with_region_final));
    log(&format!("[33] Without region: {}", final_count - with_region_final));
    log(&format!("[33] Unmatched (no country): {}", unmatched));
    ...
    let rows: Vec<(String, String, i64)> = top
        ...
        .filter_map(|r| r.ok())
        .collect();
    log("[33] Top 15 countries:");
    for (name, iso, cnt) in &rows {
        log(&format!("[33]   {} ({}) -> {}", name, iso, cnt));
    }
    ...
    let rows: Vec<(String, i64)> = top_mr
        ...
        .filter_map(|r| r.ok())
        .collect();
    log("[33] Macro regions:");
    for (mr, cnt) in &rows {
        log(&format!("[33]   {} -> {}", mr, cnt));
    }

    // Integrity check
    log("[33] Running integrity check on individuals_countries...");
    let integrity: String = conn.query_row(
        "PRAGMA integrity_check(individuals_countries)",
        [],
        |r| r.get(0),
    )?;
    log(&format!("[33] Integrity: {}", integrity));

    log("=== Step 33 complete ===");
    Ok(())
}

/// Rebuild individuals_countries with new priority order:
/// 1. Nationality (first — unchanged)
/// 2. Birth-location (second — was third)
/// 3. Death-location (third — was second)
///
/// Uses nationalities and cities tables for iso_country_name/iso_a3_code lookups.
use anyhow::Result;
...
fn main() -> Result<()> {
    ...
    log("=== Step 51: Rebuild individuals_countries (nationality->birth->death) ===");
    ...
    log("[51] Building nationality lookup...");
    ...
    log(&format!("[51] Nationality lookup: {} entries", nat_lookup.len()));

    // Build city name -> (iso_country_name, iso_a3_code) lookup
    log("[51] Building city lookup...");
    ...
    log(&format!("[51] City lookup: {} entries", city_lookup.len()));

    // ========================================================
    // PHASE 2: Drop and recreate individuals_countries table
    // ========================================================
    log("[51] Creating fresh individuals_countries table...");
    ...
    loop {
        ...
        conn.execute_batch("BEGIN TRANSACTION;")?;
        {
            ...
            for (wikidata_id, name_en, nationalities_en, birthcity_en, deathcity_en) in &batch {
                ...
                // Priority 2: birthplace (was third, now second)
                ...
                // Priority 3: deathplace (was second, now third)
                if found_country.is_none() {
        ...
        if offset % 500_000 < BATCH_SIZE as i64 {
            log(&format!(
                "[51] Progress: {}/{} processed, {} inserted (nat:{}, birth:{}, death:{}), {} unmatched",
    ...
    log("[51] === Final Statistics ===");
    log(&format!("[51] Total individuals: {}", total));
    log(&format!("[51] Total in individuals_countries: {}", final_count));
    log(&format!("[51]   via nationality: {}", matched_nationality));
    log(&format!("[51]   via birthplace: {}", matched_birth));
    log(&format!("[51]   via deathplace: {}", matched_death));
    log(&format!("[51] Unmatched (no country): {}", unmatched));
    ...
    let rows: Vec<(String, String, i64)> = top
        ...
        .filter_map(|r| r.ok())
        .collect();
    log("[51] Top 15 countries:");
    for (name, iso, cnt) in &rows {
        log(&format!("[51]   {} ({}) -> {}", name, iso, cnt));
    }

    log("=== Step 51 complete ===");
    Ok(())
}
